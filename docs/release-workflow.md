...
Flags: `--pattern`, `--file`, or `--recipe` (what to scrub), `--replace` or `--mangle` (replacement strategy), `--reason` (required, appears in commit message), `--from` or `--entire-history` (scope).
...
Direct scrubs are blocked: in rlsbl-managed repositories, safegit refuses destructive scrubs unless they run under release orchestration (`rlsbl release scrub` sets the handshake). Dry-run previews remain available.
