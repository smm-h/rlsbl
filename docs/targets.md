...
- Detection: `version.json` presence
- For specification projects where the tagged GitHub Release is the publication
- Minimal target — version.json must contain `{"version": "X.Y.Z"}`

### pgdesign

- Detection: `pgdesign.toml` in root or `schema/` subdirectory
- Version stored in `[project].version` within `pgdesign.toml`
- On release, validates schema and generates migrations if configured
...
- Conditional detection: `VERSION` file must exist AND no other target's manifest can be present
- Zero capabilities (no `read_name`, no `ci_templates`, no `dev_install`)
- Suitable for projects with no build system or package registry
