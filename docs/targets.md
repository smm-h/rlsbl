---
description: "All 17 rlsbl release targets including npm, PyPI, Go, Docker, and Flutter with auto-detection, ReleaseTarget protocol, and capabilities."
---
...
- The exclusion list: plain will not auto-detect if any of the 16 other target manifests exist (`package.json`, `pyproject.toml`, `go.mod`, `Cargo.toml`, `deno.json`, `deno.jsonc`, `mix.exs`, `build.gradle.kts`, `build.gradle`, `pom.xml`, `Package.swift`, `pubspec.yaml`, `Dockerfile`, `build.zig.zon`, `build.zig`, `version.json`, `pgdesign.toml`)
