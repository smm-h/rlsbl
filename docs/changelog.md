---
description: "JSONL changelog reference: entry schema, format_version gate, adding entries, the 12 validation checks, caching, per-version markdown generation, amending and editing entries, and pre-push enforcement."
---
...
When a single feature spans multiple commits, group them in one entry:
...
If an entry has the wrong type or description, edit it in place:
...
Add a missing entry to a past release:
...
After running `rlsbl changelog generate`:
