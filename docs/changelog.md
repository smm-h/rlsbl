...
`rlsbl changelog generate` reads all JSONL files in `.rlsbl/changes/` (sorted by semver, newest first), groups user-facing entries by type (breaking, features, fixes), and produces:
