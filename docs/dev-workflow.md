...
The `.git/hooks/pre-push` hook runs `rlsbl pre-push-check`, which enforces two things:

### JSONL commit coverage (hard error)

Every pushed commit must appear in at least one entry in `.rlsbl/changes/unreleased.jsonl`. If coverage is incomplete, the push is blocked.

Exemptions:
- Commits that only touch `.rlsbl/changes/` or `CHANGELOG.md` (changelog-only commits)
- Commits with an `Autogenerated: true` trailer (machine-generated commits from `rlsbl commit`)
- Release pushes (see below)

### Release push detection

Release pushes are auto-detected and exempted from the JSONL coverage check. Detection uses two mechanisms:

1. **Version bump commit message** -- if the push contains a commit whose message matches `^v\d+\.\d+\.\d+$`, it is recognized as a release push
2. **RLSBL_RELEASE_PUSH=1** -- set internally by `rlsbl release run` and `rlsbl release undo` in the push environment

### Manual release push warning

When a push targets a release branch (configured via `release_branches` in `.rlsbl/config.json`, defaults to `["main", "master"]`) and did NOT originate from `rlsbl release run` or `rlsbl release undo`, a warning is printed. This catches accidental manual pushes to release branches that should go through the release flow.
