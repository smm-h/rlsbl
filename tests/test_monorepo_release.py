...
class TestMonorepoRelease:
    ...
    def _setup_monorepo(self, repo_root, project_name="tooling", project_path="tooling",
        ...
        (ws_dir / "workspace.toml").write_text(
            workspace_toml(
                f'[[projects]]\npath = "{project_path}"\n'
                f'name = "{project_name}"\nreleasable = "{project_name}"\n',
                releasables=[project_name],
            )
        )

        # The releasable the member belongs to needs its own state.
        ...
        from rlsbl.workspace import get_releasable_changes_dir
        changes_dir = Path(
            get_releasable_changes_dir(str(repo_root), project_name)
        )
    ...
    def test_monorepo_reads_version_from_project_subdir(
        ...
        subprocess.run(["git", "add", "."], cwd=str(mock_git_repo), check=True)
        # The root member owns this commit, and its changelog is not the one
        # under release here -- mark it tool-authored so coverage exempts it.
        subprocess.run(
...
class TestSubtreePublish:
    ...
    def _setup_monorepo_with_subtree(self, repo_root, project_name="tooling",
        ...
        (changes_dir / "unreleased.jsonl").write_text(json.dumps({"commits": ["abc1234"], "user_facing": True, "description": "test", "type": "feature"}) + "\n")
        (proj_dir / ".rlsbl").mkdir(parents=True, exist_ok=True)
        # This suite also drives the release with workspace_root=None, which
        # reads the package's own changes dir.
        (proj_dir / ".rlsbl" / "changes").mkdir(parents=True, exist_ok=True)
        (proj_dir / ".rlsbl" / "changes" / "unreleased.jsonl").write_text(
            json.dumps({"commits": ["abc1234"], "user_facing": True,
    ...
    def test_subtree_push_failure_nonfatal(
        ...
        def mock_run_side_effect(cmd, args, **kwargs):
            ...
            # Other commands (tag -l, git tag, git push, etc.): succeed
            return ""

        mock_run.side_effect = mock_run_side_effect

        # Use workspace_root=None to avoid production scoping bug (load_workspace
