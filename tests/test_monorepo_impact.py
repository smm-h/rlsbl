...
class TestCmdImpactPackageMode:
    def test_package_input(self, tmp_path, monkeypatch, capsys):
        ...
        _cmd_impact(["models"], {}, project_root=".")
    ...
    def test_depth_flag(self, tmp_path, monkeypatch, capsys):
        monkeypatch.chdir(tmp_path)
        _make_impact_workspace(tmp_path)
        _cmd_impact(["models"], {"depth": 1}, project_root=".")
        out = capsys.readouterr().out
...
class TestCmdImpactFileMode:
    def test_file_input_maps_to_package(self, tmp_path, monkeypatch, capsys):
        ...
        _cmd_impact(["packages/models/src/money.dart"], {}, project_root=".")
        out = capsys.readouterr().out
        assert "Impact analysis for: models" in out
        assert "marketplace_contract" in out
...
class TestCmdImpactGitMode:
    def test_since_flag(self, mock_git_repo, capsys):
        ...
        _cmd_impact([], {"since": base_ref}, project_root=".")
        out = capsys.readouterr().out
        assert "Impact analysis for: models" in out
        assert "marketplace_contract" in out


class TestCmdImpactPayload:
    def test_payload_shape(self, tmp_path, monkeypatch, capsys):
        monkeypatch.chdir(tmp_path)
        _make_impact_workspace(tmp_path)
        data = _cmd_impact(["models"], {"json": True}, project_root=".")
        # Machine mode owns stdout: the human report must not be printed.
    ...
    def test_payload_leaf_package(self, tmp_path, monkeypatch, capsys):
        monkeypatch.chdir(tmp_path)
        _make_impact_workspace(tmp_path)
        data = _cmd_impact(["app"], {"json": True}, project_root=".")
