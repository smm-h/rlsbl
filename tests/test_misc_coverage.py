...
from rlsbl.commands.migrate import run_cmd as migrate_run_cmd


class TestMigrateRunCmd:
    """Tests for migrate.py run_cmd."""

    def test_migrable_not_installed(self, monkeypatch, capsys):
        monkeypatch.setattr(
            "subprocess.run",
            MagicMock(side_effect=FileNotFoundError("no migrable")),
        )
        with pytest.raises(SystemExit) as exc_info:
            migrate_run_cmd(None, [], {})
        assert exc_info.value.code == 1
        captured = capsys.readouterr()
        assert "migrable is not installed" in captured.err

    def test_status_flag_runs_migrable_status(self, monkeypatch):
        run_calls = []

        def mock_subprocess_run(cmd, **kwargs):
            run_calls.append(cmd)
            if cmd == ["migrable", "--version"]:
        ...
        with pytest.raises(SystemExit) as exc_info:
            migrate_run_cmd(None, [], {"status": True})
        assert exc_info.value.code == 0
        assert ["migrable", "status", "--config-dir", ".rlsbl"] in run_calls
    ...
    def test_dry_run_flag(self, monkeypatch):
        ...
        with pytest.raises(SystemExit) as exc_info:
            migrate_run_cmd(None, [], {"dry-run": True})
        assert exc_info.value.code == 0
        assert ["migrable", "migrate", "--dry-run", "--config-dir", ".rlsbl"] in run_calls

    def test_migrate_runs_default_command(self, monkeypatch):
        run_calls = []
        ...
        with pytest.raises(SystemExit) as exc_info:
            migrate_run_cmd(None, [], {})
        assert exc_info.value.code == 0
        assert ["migrable", "migrate", "--config-dir", ".rlsbl"] in run_calls

    def test_nonzero_exit_propagated(self, monkeypatch):
        call_count = 0
        ...
        def mock_subprocess_run(cmd, **kwargs):
            ...
            if call_count == 1:  # --version check
                return MagicMock(returncode=0)
            return MagicMock(returncode=42)  # migrable exits 42
