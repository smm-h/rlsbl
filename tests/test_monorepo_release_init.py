...
class TestCommentOutZeroCommits:
    ...
    def test_exempt_state_dir_commit_does_not_activate_a_releasable(self, mock_git_repo):
        """An exempt commit inside the releasable's own state stays uncounted.

        The count scopes first and exempts second: the commit IS inside the
        releasable's scope (its state directory belongs to no member's declared
        path), and it carries the ``Autogenerated: true`` trailer, so it is not
        an unreleased commit for this purpose and the section stays commented.
        """
        make_workspace(mock_git_repo, [
            {"path": "pkg-a", "name": "pkg-a"},
        ])
        ...
        with open(note, "w") as f:
            f.write("# regenerated\n")
        run_git(mock_git_repo, "add", note)
        run_git(
            mock_git_repo, "commit", "-q", "-m",
            "chore: regenerate\n\nAutogenerated: true",
        )

        _cmd_batch_release_init(project_root=mock_git_repo)

        raw = open(get_batch_release_file_path(str(mock_git_repo))).read()
        assert "# pkg-a: no unreleased commits since 1.0.0" in raw
        assert "pkg-a" not in tomlkit.loads(raw).get("releasables", {})
