...
class TestUndoPrModeOpen(unittest.TestCase):
    """Verify undo in PR mode when the PR is still open: closes PR + deletes branch."""

    @patch("rlsbl.commands.undo.unfinalize_release_file", return_value=[])
    @patch("rlsbl.commands.undo.unfinalize_version", return_value=[])
    @patch("rlsbl.commands.undo.get_changes_dir", return_value="/fake/.rlsbl/changes")
    @patch("rlsbl.commands.undo.find_workspace_root", return_value=None)
    @patch("rlsbl.commands.undo.is_clean_tree", return_value=True)
    @patch("rlsbl.commands.undo.check_gh_auth", return_value=True)
    @patch("rlsbl.commands.undo.check_gh_installed", return_value=True)
    @patch("rlsbl.commands.undo.run_gh")
    @patch("rlsbl.commands.undo.run")
    def test_pr_mode_open_closes_pr_and_deletes_branch(
        self, mock_run, mock_run_gh, _gh_inst, _gh_auth, _clean,
        _ws_root, _changes_dir, _unfinalize_ver, _unfinalize_rf,
    ):
        """When PR is open, undo closes the PR and deletes the release branch."""
        # run() side effects:
        # 1. git describe -> find the tag
        # 2. git rev-parse --verify refs/heads/release/v1.0.0 -> branch exists locally
        # 3. git push origin --delete release/v1.0.0 -> delete remote branch
        # 4. git branch -D release/v1.0.0 -> delete local branch
        mock_run.side_effect = [
            "v1.0.0",   # git describe --tags --abbrev=0 --match v*
            "",          # git rev-parse --verify refs/heads/release/v1.0.0 (exists)
            "",          # git push origin --delete release/v1.0.0
            "",          # git branch -D release/v1.0.0
        ]

        # run_gh() side effects:
        # 1. gh pr view release/v1.0.0 --json state --jq .state -> OPEN
        # 2. gh pr close release/v1.0.0
        mock_run_gh.side_effect = [
            "OPEN",  # gh pr view ... --jq .state
            "",      # gh pr close release/v1.0.0
        ]

        pr_mode_config = {"release": {"mode": "pr"}}

        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            run_cmd("npm", [], {"yes": True}, ctx=ProjectContext(
                project_root=Path("."), workspace_root=None, config=pr_mode_config,
            ))

        output = mock_stdout.getvalue()

        # Verify PR was closed
        mock_run_gh.assert_any_call(["pr", "close", "release/v1.0.0"], config=pr_mode_config)
        ...
        mock_run.assert_any_call("git", ["branch", "-D", "release/v1.0.0"])

        # Should NOT have done imperative undo steps (no tag deletion, no revert)
        run_args = [c[0] for c in mock_run.call_args_list]
        for c in mock_run.call_args_list:
            args = c[0]
            if len(args) >= 2 and args[0] == "git":
                git_args = args[1]
                # Should not have called git tag -d or git revert
                self.assertNotIn("tag", git_args[:1] if isinstance(git_args, list) else [])

        # Verify success message
        self.assertIn("Release PR closed and branch deleted", output)


class TestUndoPrModeMerged(unittest.TestCase):
    """Verify undo in PR mode when PR was merged: falls through to imperative undo."""
    ...
    @patch("rlsbl.commands.undo.run_gh")
    @patch("rlsbl.commands.undo.run")
    def test_pr_mode_merged_falls_through_to_imperative(
        self, mock_run, mock_run_gh, _gh_inst, _gh_auth, _clean,
        mock_branch, mock_push, _ws_root, _unfinalize_rf,
    ):
        """When PR was merged, undo falls through to normal imperative logic."""
        # run() side effects for PR state detection + imperative undo:
        import subprocess as sp

        mock_run.side_effect = [
            "v1.0.0",                              # git describe --tags
            sp.CalledProcessError(1, "git"),        # git rev-parse --verify (no local branch)
            "",                                      # git ls-remote (no remote branch)
            "",                                      # git push origin :v1.0.0
            "",                                      # git tag -d v1.0.0
            "v1.0.0",                               # git log -1 --format=%s
            "",                                      # git revert --no-edit HEAD
        ...
        # run_gh() side effects:
        # 1. gh pr view ... -> MERGED (during _detect_pr_state, no branch found)
        # 2. gh release view v1.0.0 -> success
        # 3. gh release delete v1.0.0 --yes -> success
        mock_run_gh.side_effect = [
            "MERGED",  # gh pr view ... --jq .state
        ...
        # Should have done imperative undo (deleted tag, reverted commit)
        mock_run.assert_any_call("git", ["tag", "-d", "v1.0.0"])
        mock_run.assert_any_call("git", ["revert", "--no-edit", "HEAD"])

        # Should have deleted the GitHub Release
        mock_run_gh.assert_any_call(["release", "delete", "v1.0.0", "--yes"], config=pr_mode_config)

        # Push should have been called
        mock_push.assert_called_once_with("main", env=ANY, config=pr_mode_config)

        # Verify it completed successfully
        self.assertIn("Undo complete", output)


class TestUndoPrModeNoBranch(unittest.TestCase):
    """Verify undo in PR mode when no release branch exists and no merged PR: imperative undo."""
    ...
    @patch("rlsbl.commands.undo.run_gh")
    @patch("rlsbl.commands.undo.run")
    def test_pr_mode_none_falls_through_to_imperative(
        self, mock_run, mock_run_gh, _gh_inst, _gh_auth, _clean,
        mock_branch, mock_push, _ws_root, _unfinalize_rf,
    ):
        """When no release branch and no merged PR, falls through to imperative undo."""
        import subprocess as sp
        ...
        # run_gh: pr view fails (no PR), then imperative undo steps
        mock_run_gh.side_effect = [
            sp.CalledProcessError(1, "gh"),  # gh pr view ... fails (no PR)
            "",                              # gh release view v1.0.0
            "",                              # gh release delete v1.0.0 --yes
        ]
        ...
        # Should have done imperative undo
        mock_run.assert_any_call("git", ["tag", "-d", "v1.0.0"])
        self.assertIn("Undo complete", output)


class TestDetectPrState(unittest.TestCase):
    """Unit tests for _detect_pr_state."""

    @patch("rlsbl.commands.undo.run_gh")
    @patch("rlsbl.commands.undo.run")
    def test_open_branch_exists_locally(self, mock_run, mock_run_gh):
        """Branch exists locally, PR state is OPEN."""
        mock_run.side_effect = [
            "",  # git rev-parse --verify -> success (branch exists)
        ]
        mock_run_gh.side_effect = [
            "OPEN",  # gh pr view ... -> OPEN
        ]

        from rlsbl.commands.undo import _detect_pr_state
        result = _detect_pr_state("release/v1.0.0", {})
        self.assertEqual(result, "open")

    @patch("rlsbl.commands.undo.run_gh")
    @patch("rlsbl.commands.undo.run")
    def test_merged_branch_gone(self, mock_run, mock_run_gh):
        """No branch anywhere, PR state is MERGED."""
        import subprocess as sp
        mock_run.side_effect = [
            sp.CalledProcessError(1, "git"),  # no local branch
            "",                                # ls-remote returns empty
        ]
        mock_run_gh.side_effect = [
            "MERGED",  # gh pr view -> MERGED
        ]
    ...
    @patch("rlsbl.commands.undo.run_gh")
    @patch("rlsbl.commands.undo.run")
    def test_none_no_branch_no_pr(self, mock_run, mock_run_gh):
        """No branch, no PR at all."""
        import subprocess as sp
        mock_run.side_effect = [
            sp.CalledProcessError(1, "git"),  # no local branch
            "",                                # ls-remote returns empty
        ...
        mock_run_gh.side_effect = [
            sp.CalledProcessError(1, "gh"),  # no PR found
        ]
