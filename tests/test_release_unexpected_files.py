...
class TestReleaseUnexpectedFiles:
    ...
    def test_tool_owned_scrub_result_is_not_flagged(self, tmp_project):
        """``scrub-result.json`` must be exempt from the unexpected-files guard.

        The guard whitelists ``in-progress.json`` -- the release's own state
        file -- but rlsbl owns a SECOND state file in the same directory:
        ``scrub-result.json``, the scrub resume record. A concurrent session's
        scrub (or a scrub interrupted moments earlier) drops that file into
        ...
        def scrubbing_write_version(self, dir_path, version, ctx=None):
            """Bump the version, then land a scrub resume record beside it."""
            result = original_write_version(self, dir_path, version, ctx=ctx)
            (releases_dir / "scrub-result.json").write_text('{"new_head": "x"}\n')
            return result
        ...
        with (
            ...
            patch("rlsbl.commands.release.run_gh", return_value=""),
            patch("rlsbl.commands.release.run", side_effect=fake_run),
            patch.object(NpmTarget, "write_version", scrubbing_write_version),
        ):
            run_cmd(
                _rc(),
                {"quiet": True},
        ...
        pkg = json.loads((tmp_project / "package.json").read_text())
        assert pkg["version"] == "1.0.1", (
            "the release must complete: scrub-result.json is rlsbl's own state"
        )
