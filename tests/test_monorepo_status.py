...
class TestMonorepoStatusCoverage:
    ...
    def test_a_non_releasable_member_with_its_own_changes_dir_is_refused(
        self, mock_git_repo, capsys,
    ):
        """A member outside every releasable has no changelog to report on.

        Nothing finalizes entries there and no release record explains their
        range, so status refuses the state instead of rendering a coverage
        figure for a changelog nobody owns.
        """
...
class TestMonorepoStatusUnreleasableMember:
    """A member outside every releasable has no changelog of its own."""

    def test_its_own_changes_dir_is_refused(self, mock_git_repo, capsys):
    ...
    def test_the_refusal_names_the_way_out(self, mock_git_repo, capsys):
        _unreleasable_member_workspace(mock_git_repo)
        capsys.readouterr()
        with pytest.raises(ChangelogError) as exc:
            _cmd_status({}, project_root=str(mock_git_repo))
    ...
    def test_a_commit_under_it_does_not_soften_the_refusal(
        self, mock_git_repo, capsys,
    ):
