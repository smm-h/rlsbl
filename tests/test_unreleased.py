...
class TestBaselineReleaseSelection:
    ...
    newest release, or none -- is now made from the release record, so it survives a
    ...
    def test_a_tag_with_no_archive_does_not_move_the_selection(self, mock_git_repo):
        ...
        # picked it; the release record answers from the archives, so it changes
        # nothing.
...
class TestPerProjectBaselineSelection:
    ...
    archive directory, so the scoping is structural: reading alpha's release record
...
class TestUnreleasedMonorepo:
    ...
    def test_monorepo_uses_scoped_tag(self, mock_git_repo, monkeypatch, capsys):
        ...
        # Tag alpha and record the release in alpha's own release record
    ...
    def test_monorepo_filters_commits_by_directory(self, mock_git_repo, monkeypatch, capsys):
        ...
        # Tag both, and archive each release in its releasable's release record
        subprocess.run(["git", "tag", "alpha@v1.0.0"], cwd=str(mock_git_repo), check=True)
    ...
    def test_monorepo_no_commits_for_project(self, mock_git_repo, monkeypatch, capsys):
        ...
        # Tag both, and archive each release in its project's release record
        subprocess.run(["git", "tag", "alpha@v1.0.0"], cwd=str(mock_git_repo), check=True)
        subprocess.run(["git", "tag", "beta@v1.0.0"], cwd=str(mock_git_repo), check=True)
        _tagged = git_head(mock_git_repo)
        for proj in ["alpha", "beta"]:
            archive_release(release_record_dir(mock_git_repo / proj), "1.0.0", _tagged)
