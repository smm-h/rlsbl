"""A history rewrite must move the release record's release commits, not just the changelog.
...
These tests pin the repair: the release commits go through the same map, the tree hashes
...
re-recorded archive claiming content it no longer has.
"""
...
from rlsbl.release_commit_remap import remap_release_commits
...
def _released_repo(tmp_path, *, version="1.0.0"):
    """A repo with one released version: a recorded archive plus its tag."""
    repo = tmp_path / "repo"
    init_repo(repo)
...
class TestTheAuditsReproduction:
    ...
    def test_an_unremapped_release_commit_breaks_every_release_record_read(self, tmp_path):
    ...
    def test_remapping_the_release_commit_makes_the_release_record_readable_again(self, tmp_path):
        repo, _sha = _released_repo(tmp_path)
        old, new = _rewrite(repo)

        releases = str(repo / ".rlsbl" / "releases")
        remapped = remap_release_commits(releases, {old: new}, cwd=str(repo))
    ...
    def test_the_archive_stays_read_only_and_valid(self, tmp_path):
        ...
        old, new = _rewrite(repo)
        releases = str(repo / ".rlsbl" / "releases")
        remap_release_commits(releases, {old: new}, cwd=str(repo))
...
class TestWhatIsLeftAlone:
    ...
    def test_a_release_commit_the_map_does_not_mention_is_untouched(self, tmp_path):
        repo, sha = _released_repo(tmp_path)
        releases = str(repo / ".rlsbl" / "releases")
        assert remap_release_commits(releases, {"0" * 40: "1" * 40},
                                     cwd=str(repo)) == []
        assert read_release_file(
            archived_release_path(releases, "1.0.0")
        ).candidate_sha == sha

    def test_an_unrecoverable_archive_is_skipped(self, tmp_path):
        repo, _sha = _released_repo(tmp_path)
        releases = repo / ".rlsbl" / "releases"
        write_archived_release_file(
            str(releases), "0.9.0",
        ...
        old, new = _rewrite(repo)
        remapped = remap_release_commits(str(releases), {old: new}, cwd=str(repo))
