...
class TestRelease:
    ...
    def test_release_fetch_failure_warns_but_continues(self, _validate, _gen_cl,
        ...
        with patch("sys.stdout", new_callable=StringIO):
            run_cmd(_rc(), {"quiet": False, "dry-run": True}, ctx=ProjectContext(project_root=Path("."), workspace_root=None, config={"private": False}))
...
class TestReleaseCommitTrailers:
    ...
    def test_version_bump_commit_is_autogenerated(self, _changes_dir, _extract, _finalize,
        ...
        with patch("sys.stdout", new_callable=StringIO):
            run_cmd(_rc(), {"yes": True, "quiet": False}, ctx=ProjectContext(project_root=Path("."), workspace_root=None, config={"private": False}))
