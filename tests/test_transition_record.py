"""Tests for committed transition records -- the append-only log of repository-surgery facts.
...
from rlsbl import transition_record
...
def make_departed_globs():
    return DepartedGlobsEvent(
        ...
        destination=TransitionRecordEndpoint(repo="https://github.com/example/widget"),
    )
    ...
    def test_caller_supplied_id_is_preserved(self, tmp_path):
        ...
        written = append_event(
            path, ConversionEvent(
                ...
                direction="absorb",
                source=TransitionRecordEndpoint(repo="https://github.com/example/widget"),
                destination=TransitionRecordEndpoint(repo=".", path="packages/widget"),
                commit=SHA_A,
            ),
        )
...
class TestLocation:
    def test_standalone_repo_location(self, tmp_path):
        path = get_transition_record_path(str(tmp_path))
        assert path == os.path.join(str(tmp_path), ".rlsbl", "transitions.jsonl")

    def test_releasable_location(self, tmp_path):
        releasable_dir = get_releasable_dir(str(tmp_path), "widget")
        path = get_transition_record_path(str(tmp_path), releasable_dir=releasable_dir)
    ...
    def test_workspace_location(self, tmp_path):
        ...
        home of its own.
        """
        path = get_transition_record_path(str(tmp_path), workspace=True)
        assert path == os.path.join(
            str(tmp_path), ".rlsbl-monorepo", "transitions.jsonl",
        )
    ...
    def test_both_locations_hold_the_same_format(self, tmp_path):
        ...
        solo_path = get_transition_record_path(str(standalone))
        mono_path = get_transition_record_path(
    ...
    def test_absent_record_reads_as_no_events(self, tmp_path):
        path = get_transition_record_path(str(tmp_path))
        assert not transition_record_file_exists(path)
    ...
    def test_exists_never_reads_content(self, tmp_path):
        ...
        assert transition_record_file_exists(path) is True
...
class TestMalformedRecordsAreHardErrors:
    ...
    def _expect(self, tmp_path, payload_or_line, *, needle):
        ...
        write_raw(path, line)
        with pytest.raises(TransitionRecordError) as exc:
            read_events(path)
...
class TestModuleSurface:
    ...
    def test_generated_validator_is_paired_with_the_runtime(self):
        ...
        from rlsbl.strictspec_gen import transition_record_event_validator as validator
