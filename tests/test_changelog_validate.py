...
from rlsbl.release_record import range_anchor, unreleased_range
...
def git_repo(tmp_path, monkeypatch):
    ...
    # range has an anchor. The archive, not the tag, is what bounds the range.
...
class TestRangeAnchorSelection:
    """Which release the release record anchors the unreleased range to.
    ...
    def test_no_releases_returns_none(self, git_repo):
        """With nothing archived, there is no anchor."""
        assert range_anchor(_release_record_dir(git_repo)) is None

    def test_archived_release_is_the_anchor(self, git_repo):
        sha = _git_head(git_repo)
        _run_git(git_repo, "tag", "v0.1.0")
        _archive_release(_release_record_dir(git_repo), "0.1.0", sha)
        anchor = range_anchor(_release_record_dir(git_repo))
    ...
    def test_a_stray_tag_above_the_newest_release_anchors_nothing(self, git_repo):
        ...
        assert range_anchor(_release_record_dir(git_repo)).version == "0.1.0"
    ...
    def test_version_tags_over_an_empty_release_record_are_a_hard_error(self, git_repo):
        ...
        with pytest.raises(ReleaseRecordError, match="backfill_release_anchors"):
            range_anchor(_release_record_dir(git_repo))
    ...
    def test_each_project_reads_its_own_release_record(self, git_repo):
        ...
        anchor = range_anchor(_release_record_dir(None, releasable_dir=mine),
                              tag_glob="mylib@v*")
        assert anchor.version == "0.1.0"
    ...
    def test_picks_the_highest_archived_version(self, git_repo):
        ...
        anchor = range_anchor(releases, tag_glob="mylib@v*")
        assert anchor.version == "0.2.0"
...
class TestRangeFunctionsLiveWhereTheyBelong:
    ...
    def test_range_anchor_comes_from_the_release_record(self, git_repo):
        anchor = range_anchor(_release_record_dir(git_repo))
        assert anchor.version == "0.0.0"
