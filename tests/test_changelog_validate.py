...
from rlsbl.release_record import nearest_release_commit, unreleased_range
...
def git_repo(tmp_path, monkeypatch):
    ...
    # Create a baseline version tag AND its release record entry, so the unreleased
    # range has a release commit. The archive, not the tag, is what bounds the range.
...
class TestCheckInRange:
    ...
    def test_passes_when_hash_in_range(self, git_repo):
        sha = _make_commit(git_repo)
        entries = [ChangelogEntry(commits=[sha], user_facing=False)]
        passed, details = check_in_range(entries, _release_record_dir(git_repo))
        assert passed is True
        assert details == []
...
class TestCheckNoOrphans:
    ...
    def test_passes_with_valid_entries(self, git_repo):
        sha = _make_commit(git_repo)
        entries = [ChangelogEntry(commits=[sha], user_facing=False)]
        passed, details = check_no_orphans(entries, _release_record_dir(git_repo))
        assert passed is True
...
class TestRangeReleaseCommitSelection:
    """Which release the release record bounds the unreleased range to.

    These pinned ``git describe --tags --abbrev=0 --match <glob>`` before the
    release record existed. They now pin the same SELECTION -- newest release, scoped
    to the project's own scheme -- made from the release archives instead, so
    a deleted or hand-made tag cannot move it.
    """

    @pytest.fixture
    def git_repo(self, tmp_path, monkeypatch):
        """A git repo with an initial commit and NO release archived."""
        repo = tmp_path / "repo"
        repo.mkdir()
        monkeypatch.chdir(repo)
    ...
    def test_no_releases_returns_none(self, git_repo):
        """With nothing archived, there is no release commit."""
        assert nearest_release_commit(_release_record_dir(git_repo)) is None

    def test_archived_release_is_the_release_commit(self, git_repo):
        sha = _git_head(git_repo)
        _run_git(git_repo, "tag", "v0.1.0")
        _archive_release(_release_record_dir(git_repo), "0.1.0", sha)
        release_commit = nearest_release_commit(_release_record_dir(git_repo))
        assert (release_commit.version, release_commit.candidate_sha) == ("0.1.0", sha)

    def test_a_stray_tag_above_the_newest_release_bounds_nothing(self, git_repo):
        """The tag namespace alone no longer selects anything.

        `git describe` answered "v9.9.9" here. The release record answers with the
        newest ARCHIVED release, because that is what shipped.
        """
        sha = _git_head(git_repo)
        _archive_release(_release_record_dir(git_repo), "0.1.0", sha)
        _run_git(git_repo, "tag", "v9.9.9")
        assert nearest_release_commit(_release_record_dir(git_repo)).version == "0.1.0"
    ...
    def test_each_project_reads_its_own_release_record(self, git_repo):
        ...
        sibling's releases are invisible without any glob matching at all."""
        sha = _git_head(git_repo)
        mine = git_repo / ".rlsbl-monorepo" / "releasables" / "mylib"
        theirs = git_repo / ".rlsbl-monorepo" / "releasables" / "other"
        _archive_release(_release_record_dir(None, releasable_dir=mine), "0.1.0", sha)
        _archive_release(_release_record_dir(None, releasable_dir=theirs), "9.9.9", sha)
        release_commit = nearest_release_commit(_release_record_dir(None, releasable_dir=mine),
                              tag_glob="mylib@v*")
        assert release_commit.version == "0.1.0"

    def test_picks_the_highest_archived_version(self, git_repo):
        releases = _release_record_dir(git_repo)
        _archive_release(releases, "0.1.0", _git_head(git_repo))
        newer = _make_commit(git_repo)
        _archive_release(releases, "0.2.0", newer)
        release_commit = nearest_release_commit(releases, tag_glob="mylib@v*")
        assert release_commit.version == "0.2.0"


class TestUnreleasedRange:
    """The range spec the release record produces, replacing ``<tag>..HEAD``."""
    ...
    def test_no_releases_returns_head(self, git_repo):
        """With nothing released, returns 'HEAD' (all commits, first release)."""
        assert unreleased_range(_release_record_dir(git_repo)) == "HEAD"

    def test_with_an_archived_release(self, git_repo):
        """The range starts at the released COMMIT, not at its tag name."""
        sha = _git_head(git_repo)
        _run_git(git_repo, "tag", "v0.1.0")
        _archive_release(_release_record_dir(git_repo), "0.1.0", sha)
        assert unreleased_range(_release_record_dir(git_repo)) == f"{sha}..HEAD"

    def test_release_survives_its_tag_being_deleted(self, git_repo):
        """The one thing the tag-based range could not do."""
        sha = _git_head(git_repo)
        _run_git(git_repo, "tag", "v0.1.0")
        _archive_release(_release_record_dir(git_repo), "0.1.0", sha)
...
class TestMonorepoValidation:
    ...
    def test_omitting_the_tag_glob_no_longer_widens_the_range(self, monorepo_fixture):
        """A missing tag glob cannot silently swallow a monorepo's history.

        This test used to assert the opposite, and the assertion was the bug:
        with the range coming from ``git describe --match 'v*'``, a monorepo
        member whose tags read ``mypylib@v0.1.0`` matched nothing, the range
        fell back to all of HEAD, and every commit since the repo's first
        looked uncovered.  The release record is read from the member's OWN archive
        directory, so the glob is not what scopes it -- it only names the tag
        scheme the archives are cross-checked against.
        """
        root = monorepo_fixture.root
        python_dir = monorepo_fixture.python_dir
        ...
        # (Order matters only in that a successful validation commits its own
        # cache file, so the second run sees one more exempt commit; the
        # PASS/FAIL verdict is what is being compared.)
        with_glob = validate_unreleased(changes_dir, tag_glob="mypylib@v*", config={})
        without_glob = validate_unreleased(changes_dir, config={})
        assert with_glob["checks"]["coverage"][0] is True
        assert without_glob["checks"]["coverage"][0] is True
        assert not any(
            "not covered" in d for d in without_glob["checks"]["coverage"][1]
        )
...
class TestMonorepoPathScoping:
    ...
    def monorepo_repo(self, tmp_path, monkeypatch):
        ...
        # Tag for pkg-a baseline, and archive it -- the range comes from the
        # release record beside pkg-a's changes dir, not from the tag.
        _run_git(repo, "tag", "pkg-a@v0.1.0")
        _archive_release(_release_record_dir(repo / "pkg-a"), "0.1.0", _git_head(repo))
    ...
    def test_coverage_only_requires_project_commits(self, monorepo_repo):
        ...
        passed, details = check_coverage(entries, _release_record_dir(repo / "pkg-a"), tag_glob="pkg-a@v*", scope=scope)
        assert passed is True
    ...
    def test_coverage_without_project_requires_all_commits(self, monorepo_repo):
        ...
        entries = [ChangelogEntry(commits=[sha_a], user_facing=False)]
        passed, details = check_coverage(entries, _release_record_dir(repo / "pkg-a"), tag_glob="pkg-a@v*", scope=None)
        assert passed is False
        assert any("not covered" in d for d in details)
    ...
    def test_in_range_with_project_scoping(self, monorepo_repo):
        ...
        entries = [ChangelogEntry(commits=[sha_a], user_facing=False)]
        passed, details = check_in_range(entries, _release_record_dir(repo / "pkg-a"), tag_glob="pkg-a@v*", scope=scope)
        assert passed is True
...
class TestRangeFunctionsLiveWhereTheyBelong:
    """Where the range machinery is imported from.

    Range SELECTION moved out of ``changelog.resolve`` into ``rlsbl.release_record``
    when the archives replaced ``git describe`` as the authority; hash
    resolution stayed. This pins the split so neither drifts back."""

    def test_range_release_commit_comes_from_the_release_record(self, git_repo):
        release_commit = nearest_release_commit(_release_record_dir(git_repo))
        assert release_commit.version == "0.0.0"

    def test_unreleased_range_comes_from_the_release_record(self, git_repo):
        release_commit = nearest_release_commit(_release_record_dir(git_repo))
        assert unreleased_range(_release_record_dir(git_repo)) == (
            f"{release_commit.candidate_sha}..HEAD"
        )

    def test_resolve_no_longer_carries_range_selection(self):
        import rlsbl.changelog.resolve as resolve_mod

        assert not hasattr(resolve_mod, "_unreleased_range")
        assert not hasattr(resolve_mod, "_get_last_version_tag")
