...
def git_repo(tmp_path, monkeypatch):
    ...
    # Create a baseline version tag AND its ledger entry, so the unreleased
    # range has an anchor. The archive, not the tag, is what bounds the range.
    _run_git(repo, "tag", "v0.0.0")
    _archive_release(_ledger_dir(repo), "0.0.0", _git_head(repo))
...
class TestCheckInRange:
    ...
    def test_passes_when_hash_in_range(self, git_repo):
        sha = _make_commit(git_repo)
        entries = [ChangelogEntry(commits=[sha], user_facing=False)]
        passed, details = check_in_range(entries, _ledger_dir(git_repo))
        assert passed is True
        assert details == []
...
class TestCheckCoverage:
    ...
    def test_passes_when_all_covered(self, git_repo):
        ...
        passed, details = check_coverage(entries, _ledger_dir(git_repo))
        assert passed is True
...
class TestCheckNoOrphans:
    ...
    def test_passes_with_valid_entries(self, git_repo):
        sha = _make_commit(git_repo)
        entries = [ChangelogEntry(commits=[sha], user_facing=False)]
        passed, details = check_no_orphans(entries, _ledger_dir(git_repo))
        assert passed is True
...
class TestRangeAnchorSelection:
    """Which release the ledger anchors the unreleased range to.

    These pinned ``git describe --tags --abbrev=0 --match <glob>`` before the
    ledger existed. They now pin the same SELECTION -- newest release, scoped
    ...
    def test_each_project_reads_its_own_ledger(self, git_repo):
        ...
        _archive_release(_ledger_dir(None, releasable_dir=mine), "0.1.0", sha)
        _archive_release(_ledger_dir(None, releasable_dir=theirs), "9.9.9", sha)
        anchor = range_anchor(_ledger_dir(None, releasable_dir=mine),
                              tag_glob="mylib@v*")
        assert anchor.version == "0.1.0"

    def test_picks_the_highest_archived_version(self, git_repo):
        releases = _ledger_dir(git_repo)
...
class TestUnreleasedRange:
    """The range spec the ledger produces, replacing ``<tag>..HEAD``."""
...
class TestMonorepoValidation:
    ...
    def test_omitting_the_tag_glob_no_longer_widens_the_range(self, monorepo_fixture):
        ...
        looked uncovered.  The ledger is read from the member's OWN archive
...
class TestMonorepoPathScoping:
    ...
    def monorepo_repo(self, tmp_path, monkeypatch):
        ...
        # ledger beside pkg-a's changes dir, not from the tag.
        _run_git(repo, "tag", "pkg-a@v0.1.0")
        _archive_release(_ledger_dir(repo / "pkg-a"), "0.1.0", _git_head(repo))
    ...
    def test_coverage_only_requires_project_commits(self, monorepo_repo):
        ...
        passed, details = check_coverage(entries, _ledger_dir(repo / "pkg-a"), tag_glob="pkg-a@v*", scope=scope)
        assert passed is True
    ...
    def test_coverage_without_project_requires_all_commits(self, monorepo_repo):
        ...
        entries = [ChangelogEntry(commits=[sha_a], user_facing=False)]
        passed, details = check_coverage(entries, _ledger_dir(repo / "pkg-a"), tag_glob="pkg-a@v*", scope=None)
        assert passed is False
        assert any("not covered" in d for d in details)
    ...
    def test_in_range_with_project_scoping(self, monorepo_repo):
        ...
        entries = [ChangelogEntry(commits=[sha_a], user_facing=False)]
        passed, details = check_in_range(entries, _ledger_dir(repo / "pkg-a"), tag_glob="pkg-a@v*", scope=scope)
        assert passed is True
...
class TestRangeFunctionsLiveWhereTheyBelong:
    ...
    Range SELECTION moved out of ``changelog.resolve`` into ``rlsbl.ledger``
    ...
    def test_range_anchor_comes_from_the_ledger(self, git_repo):
        anchor = range_anchor(_ledger_dir(git_repo))
        assert anchor.version == "0.0.0"

    def test_unreleased_range_comes_from_the_ledger(self, git_repo):
        anchor = range_anchor(_ledger_dir(git_repo))
        assert unreleased_range(_ledger_dir(git_repo)) == (
