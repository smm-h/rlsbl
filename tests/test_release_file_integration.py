...
Verifies that:
- run_cmd(ReleaseConfig, flags, ctx=ProjectContext(project_root=Path("."), workspace_root=None, config={"publish_mode": "ci", "pipelines": {}})) works (dry-run mode)
...
class TestRunCmdWithReleaseConfig:
    """run_cmd(ReleaseConfig, flags, ctx=ProjectContext(project_root=Path("."), workspace_root=None, config={"publish_mode": "ci", "pipelines": {}})) works correctly in dry-run mode."""
    ...
    def test_dry_run_with_release_config(
        ...
        run_cmd(config, {"dry-run": True, "quiet": False, "yes": True}, ctx=ProjectContext(project_root=Path("."), workspace_root=None, config={"publish_mode": "ci", "pipelines": {}}))
...
class TestTargetExhaustivenessValidation:
    ...
    def test_missing_target_in_release_file(
        ...
        with pytest.raises(SystemExit) as exc_info:
            run_cmd(config, {"dry-run": True, "quiet": True, "yes": True}, ctx=ProjectContext(project_root=Path("."), workspace_root=None, config={"publish_mode": "ci", "pipelines": {}}))
...
class TestCmdReleaseMissingFile:
    ...
    def test_no_release_file_gives_error(self, tmp_project, capsys):
        ...
        with pytest.raises(SystemExit) as exc_info:
            cmd_release_run(cli_ctx(yes=True, quiet=True), allow_dirty=False, watch=False, bump="", description="", push_timeout=0, ci_timeout=0, check_timeout=0, hook_timeout=0, preid="")
...
class TestReleaseConfigSignature:
    """run_cmd(ReleaseConfig, flags, ctx=ProjectContext(project_root=Path("."), workspace_root=None, config={"publish_mode": "ci", "pipelines": {}})) is the only supported calling convention."""
    ...
    def test_run_cmd_with_release_config(
        ...
        """run_cmd(ReleaseConfig, flags, ctx=ProjectContext(project_root=Path("."), workspace_root=None, config={"publish_mode": "ci", "pipelines": {}})) works in dry-run mode."""
        _setup_npm_project(tmp_project)
        mock_run.side_effect = ["", "0", "", ""]
...
class TestMonorepoDirectoryScoping:
    ...
    def test_validate_unreleased_receives_project_dict(
        ...
        run_cmd(config, {"dry-run": True, "quiet": False, "yes": True}, ctx=ProjectContext(project_root=Path("."), workspace_root=Path(str(monorepo_fixture.root)), config={"publish_mode": "ci", "pipelines": {}}))
