...
Verifies that:
- run_cmd(ReleaseConfig, flags, ctx=ProjectContext(project_root=Path("."), workspace_root=None, config={"private": False, "pipelines": {}})) works (dry-run mode)
...
class TestRunCmdWithReleaseConfig:
    ...
    def test_dry_run_with_release_config(
        ...
        run_cmd(config, {"dry-run": True, "quiet": False, "yes": True}, ctx=ProjectContext(project_root=Path("."), workspace_root=None, config={"private": False, "pipelines": {}}))
...
class TestTargetExhaustivenessValidation:
    ...
    def test_missing_target_in_release_file(
        ...
        with pytest.raises(SystemExit) as exc_info:
            run_cmd(config, {"dry-run": True, "quiet": True, "yes": True}, ctx=ProjectContext(project_root=Path("."), workspace_root=None, config={"private": False, "pipelines": {}}))
