...
def get_push_timeout(project_root):
...
def push_if_needed(branch, env=None, *, project_root):
    ...
    Args:
        ...
        project_root: project root path forwarded to get_push_timeout.
    """
...
def commit_files(
    ...
    try:
        ...
        if tool == "safegit":
            run(tool, ["commit", *trailer_args, "-m", message, "--", *files])
        else:
            run("git", ["add", *files])
            run("git", ["commit", *trailer_args, "-m", message])
        return True
...
def commit_files_if_changed(
    ...
    if not status:
        print(skip_message)
        return False
    commit_files(message, files, allow_failure=False, autogenerated=autogenerated)
    return True
