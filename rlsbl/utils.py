...
def commit_files(
    ...
    When autogenerated is True, appends an "Autogenerated: true" trailer to the
    commit message body. Git's trailer parser recognizes this format natively.
    ...
    try:
        ...
        if tool == "safegit":
            run(tool, ["commit", "-m", commit_message, "--", *files])
        else:
            run("git", ["add", *files])
