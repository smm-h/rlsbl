...
def _get_unreleased_commit_count(proj, workspace_root, all_projects):
    ...
    Uses the project's monorepo tag glob to find the last version tag, then
    counts commits since that tag that touch a file *proj* owns.  Ownership is
    resolved against *all_projects* -- the whole member list -- so a file under
    a nested member counts for that member alone.  Returns (count, last_tag)
    where last_tag is the tag string or None if no tags exist.
    """
...
def _scaffold_releasable_sections(workspace_root, projects, batch_path, filter_names):
    ...
    for rel in releasables:
        ...
        for proj in member_projs:
            count, tag = _get_unreleased_commit_count(proj, workspace_root, projects)
            total_commits += count
            if tag is not None:
                last_tag = tag
...
def _scaffold_package_sections(workspace_root, projects, batch_path, filter_names):
    ...
    for proj in projects:
        ...
        commit_count, last_tag = _get_unreleased_commit_count(proj, workspace_root, projects)
