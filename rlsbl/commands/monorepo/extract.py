...
def _commit_extracted_state(repo):
    ...
    """
    paths = working_tree_paths(cwd=str(repo))
    if not paths:
        return
    commit_files(
...
def _migrate_changelog_to_new_repo(
    ...
    """
    effects.makedirs(target_changes_dir, exist_ok=True)
    files_written = 0
    entries_migrated = 0
    ...
    if unreleased_entries:
        ...
        if filtered:
            ...
            with effects.open_write(target_unreleased, "w", encoding="utf-8") as f:
    ...
    for version_str, version_path in versioned:
        ...
        if filtered:
            ...
            with effects.open_write(target_version, "w", encoding="utf-8") as f:
                for entry in filtered:
                    f.write(serialize_entry(entry) + "\n")
            files_written += 1
            entries_migrated += len(filtered)
...
def _create_rlsbl_config(target_path, source_config_path=None):
    ...
    effects.makedirs(rlsbl_dir, exist_ok=True)
