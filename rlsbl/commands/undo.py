...
from ..utils import run, gh_env, check_gh_installed, check_gh_auth, get_push_timeout, get_current_branch, push_if_needed, is_clean_tree
...
def run_cmd(registry, args, flags, *, ctx):
    ...
    try:
        ...
        try:
            run("gh", ["release", "delete", tag, "--yes"], env=gh_env(ctx.config))
