...
def run_cmd(project_root):
    ...
    doc.add(tomlkit.comment('  prerelease: advance an existing pre-release (0.6.0-beta.0 -> -beta.1)'))
    doc.add("bump", "")
    ...
    doc.add("context", "")
    doc.add(tomlkit.comment("Pre-release channel: alpha, beta, rc, or stable (ordered;"))
    doc.add(tomlkit.comment("demotion is an error). With patch/minor/major it ENTERS the"))
    doc.add(tomlkit.comment('channel (0.5.2 + minor + alpha -> 0.6.0-alpha.0); with'))
    doc.add(tomlkit.comment('bump = "prerelease" it promotes, and "stable" strips the'))
    doc.add(tomlkit.comment("suffix. See docs/release-workflow.md#the-pre-release-channel."))
