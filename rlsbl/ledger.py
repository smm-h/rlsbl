...
def read_entry(releases_dir: str, version: str, *, tag_glob: str | None = None,
    ...
    if tag_commit is not None and not _same_commit(tag_commit, sha):
        raise LedgerError(
            ...
            f"  archive: {path}\n"
            f"  The archive is the record of what shipped and is read-only from\n"
            f"  the instant the release wrote it, so a disagreement means the TAG\n"
            f"  moved -- a history rewrite that did not re-point it, or a hand-made\n"
            f"  tag on the wrong commit. Re-point the tag at the anchor "
            f"(git tag -f\n"
