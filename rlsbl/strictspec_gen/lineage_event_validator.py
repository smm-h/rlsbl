...
# schema:              rlsbl-lineage-event (format_version 1)
...
class LineageEndpoint:
    """Frozen typed binding of the "LineageEndpoint" record. Immutable; use with_* for
    copy-on-write.
    """
    ...
    def with_repo(self, v: str) -> LineageEndpoint:
        return replace(self, repo=v)

    def with_path(self, v: str) -> LineageEndpoint:
        return replace(self, path=v)

    def with_project(self, v: str) -> LineageEndpoint:
        return replace(self, project=v)

    def with_releasable(self, v: str) -> LineageEndpoint:
        return replace(self, releasable=v)

    def with_tag_format(self, v: str) -> LineageEndpoint:
        return replace(self, tag_format=v)


def _bind_LineageEndpoint(v: Value) -> LineageEndpoint | None:
...
@dataclass(frozen=True, kw_only=True)
class AnchorMapping:
    """Frozen typed binding of the "AnchorMapping" record. Immutable; use with_* for
    copy-on-write.
    """
    ...
    def with_old_sha(self, v: str) -> AnchorMapping:
        return replace(self, old_sha=v)

    def with_new_sha(self, v: str) -> AnchorMapping:
        return replace(self, new_sha=v)


def _bind_AnchorMapping(v: Value) -> AnchorMapping | None:
    if v.kind() != strictspec.Kind.RECORD:
        return None
...
class ConversionEvent:
    ...
    def with_source(self, v: LineageEndpoint) -> ConversionEvent:
        return replace(self, source=v)

    def with_destination(self, v: LineageEndpoint) -> ConversionEvent:
...
def _bind_ConversionEvent(v: Value) -> ConversionEvent | None:
    ...
    return ConversionEvent(
        ...
        source=(_bind_LineageEndpoint(f_source[0]) if f_source[1] else None),
        destination=(_bind_LineageEndpoint(f_destination[0]) if f_destination[1] else None),
        commit=(f_commit[0].string()[0] if f_commit[1] else ""),
    )
...
@dataclass(frozen=True, kw_only=True)
class AnchorRemapEvent:
    """Frozen typed binding of the "AnchorRemapEvent" record. Immutable; use with_* for
    copy-on-write.
    """
    ...
    def with_format_version(self, v: int) -> AnchorRemapEvent:
        return replace(self, format_version=v)

    def with_id(self, v: str) -> AnchorRemapEvent:
        return replace(self, id=v)

    def with_recorded_at(self, v: str) -> AnchorRemapEvent:
        return replace(self, recorded_at=v)

    def with_kind(self, v: str) -> AnchorRemapEvent:
        return replace(self, kind=v)

    def with_related_to(self, v: str) -> AnchorRemapEvent:
        return replace(self, related_to=v)

    def with_rewrite(self, v: str) -> AnchorRemapEvent:
        return replace(self, rewrite=v)

    def with_mappings(self, v: list[AnchorMapping]) -> AnchorRemapEvent:
        return replace(self, mappings=v)


def _bind_AnchorRemapEvent(v: Value) -> AnchorRemapEvent | None:
    if v.kind() != strictspec.Kind.RECORD:
        return None
    f_format_version = v.field("format_version")
    f_id = v.field("id")
    ...
    return AnchorRemapEvent(
        ...
        mappings=([_bind_AnchorMapping(e) for e in f_mappings[0].items()] if f_mappings[1] else []),
    )
...
class DepartedGlobsEvent:
    ...
    def with_destination(self, v: LineageEndpoint) -> DepartedGlobsEvent:
        return replace(self, destination=v)
