...
def check_in_range(entries: list[ChangelogEntry], tag_glob: str | None = None, scope=None) -> tuple[bool, list[str]]:
...
def check_coverage(entries: list[ChangelogEntry], tag_glob: str | None = None, scope=None) -> tuple[bool, list[str]]:
    ...
    When tag_glob is set, scopes to monorepo tags. When *scope* is set
    (monorepo mode), only commits touching files the scope's members own
    require coverage -- a commit whose every file belongs to some other
    member is filtered out.  *scope* is an
    :class:`~rlsbl.ownership.OwnershipScope`.
    """
    details: list[str] = []
    unreleased_commits = set(_git_log_hashes(_unreleased_range(tag_glob)))
...
def validate_unreleased(changes_dir: str, tag_glob: str | None = None, scope=None, *, config: dict, bump_type: str | None = None) -> dict:
    ...
    *scope* is an :class:`~rlsbl.ownership.OwnershipScope`.
    ...
    checks = {
        ...
        "in_range": check_in_range(entries, tag_glob, scope=scope),
        "coverage": check_coverage(entries, tag_glob, scope=scope),
        "no_orphans": check_no_orphans(entries, tag_glob, scope=scope),
