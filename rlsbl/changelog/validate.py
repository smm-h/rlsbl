...
def check_in_range(entries: list[ChangelogEntry], releases_dir: str,
    ...
    Unreleased range: the commits since the release the LEDGER anchors this
    checkout to -- the highest archived version whose ``candidate_sha`` this
    history contains -- or all commits when the ledger records none.
    ``releases_dir`` is that ledger; ``tag_glob`` names the tag scheme, used
...
def check_coverage(entries: list[ChangelogEntry], releases_dir: str,
    ...
    The unreleased range comes from the LEDGER at ``releases_dir`` -- see
...
def check_no_orphans(
    ...
    unresolvable or outside the unreleased range (the LEDGER's range at
...
def validate_unreleased(changes_dir: str, tag_glob: str | None = None, scope=None, *, config: dict, bump_type: str | None = None) -> dict:
    ...
    skips full revalidation. The unreleased range comes from the LEDGER --
