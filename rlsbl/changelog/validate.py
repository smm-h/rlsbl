...
def filter_exempt_commits(commits: list[str]) -> tuple[list[str], ExemptionStats]:
    ...
    1. ``_is_autogenerated`` (commit has ``Autogenerated: true`` trailer)
    2. ``_is_changelog_only_commit`` (commit only touches changelog files)
    ...
    for commit in commits:
        ...
        elif _is_changelog_only_commit(commit):
