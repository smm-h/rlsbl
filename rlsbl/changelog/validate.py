...
def _filter_commits_for_scope(commits, project):
    ...
    When ``project`` is a list (releasable members in explicit mode),
    delegates to ``filter_commits_for_releasable``.  When it is a single
    project dict, delegates to ``filter_commits_for_project``.  Returns
    the input unchanged when ``project`` is None (standalone mode).
    """
    if project is None:
        return commits
    if isinstance(project, list):
        return filter_commits_for_releasable(commits, project)
    return filter_commits_for_project(commits, project)
...
def check_in_range(entries: list[ChangelogEntry], tag_glob: str | None = None, project=None) -> tuple[bool, list[str]]:
    ...
    project's files are considered in-range.  ``project`` may be a single
    project dict or a list of projects (releasable members).
    """
    details: list[str] = []
    unreleased_commits = set(_git_log_hashes(_unreleased_range(tag_glob)))

    unreleased_commits = _filter_commits_for_scope(unreleased_commits, project)
...
def check_coverage(entries: list[ChangelogEntry], tag_glob: str | None = None, project=None) -> tuple[bool, list[str]]:
    ...
    ``project`` may be a single project dict or a list of projects
    (releasable members).
    """
    details: list[str] = []
    unreleased_commits = set(_git_log_hashes(_unreleased_range(tag_glob)))
    ...
    if project is not None:
        project_commits = _filter_commits_for_scope(unreleased_commits, project)
...
def check_no_orphans(
    ...
    valid and in range.  ``project`` may be a single project dict or a
    list of projects (releasable members).
    """
    details: list[str] = []
    unreleased_commits = set(_git_log_hashes(_unreleased_range(tag_glob)))
...
def validate_unreleased(changes_dir: str, tag_glob: str | None = None, project=None, *, config: dict, bump_type: str | None = None) -> dict:
    ...
    checks only consider commits touching the project's files.  ``project``
    may be a single project dict or a list of projects (releasable members).
