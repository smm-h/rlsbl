...
- ``x.y.z.md`` -- a regenerated derivative of the JSONL. Normally writable,
  ...
  when it must rewrite a locked .md, temporarily unlocks it and restores the
  file's ORIGINAL mode afterwards (never assuming 444), so a deliberate lock
...
from .files import get_changes_dir, is_read_only, list_versioned_files, read_unreleased
...
@contextmanager
def _writable_preserving_mode(path: str):
    """Temporarily make a read-only file writable, restoring its EXACT mode.

    Unlike ``writable_jsonl`` (which always relocks to 0o444, the immutable
    finalized-JSONL convention), this preserves whatever mode the file had.
    A per-version .md is a derivative that may be locked to any read-only
    mode; regeneration must not silently change that mode. A missing path is
    a no-op (the caller is creating a new file).
    """
    exists = os.path.exists(path)
    orig_mode = stat.S_IMODE(os.stat(path).st_mode) if exists else None
    unlocked = exists and is_read_only(path)
    if unlocked:
    ...
    if write_to_disk:
        ...
        if os.path.exists(md_path):
            with open(md_path, "r", encoding="utf-8") as f:
                existing = f.read()
        # Compare-first (above) happens BEFORE any open-for-write, so unchanged
        # content is a pure no-op: no unlock, no write, mtime untouched. When
        # content differs and the .md is locked read-only, unlock it, write,
        # and restore its original mode.
        if existing != md:
            with _writable_preserving_mode(md_path):
